...
unid_vertex_par="${tmp_db}.unid_vertex_par"
mid_mapping="${tmp_db}.old_new_unid_mapping_par"
...
hive -e "
create table if not exists $unid_vertex_par(
id1 string,
id2 string
) partitioned by (
month string,
...
--class com.mob.mid_full_v2.MidStep1Pkg2VertexRnid \
...
./Mid2.0.jar  1000 $month $version $unid_vertex_par
fi
...
./Mid2.0.jar $month $version 10 $unid_vertex_par $mid_mapping
