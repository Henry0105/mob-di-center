...
object Car4sPinyinMatch {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val shoppingMallNameWord = spark.sql(
      s"""
         |select name
         |from $database.car_4s_ssid_calculate_base_info
         |where day='$day'
    ...
    // join得到4s店名字拼音和简写拼音、所在城市地区的拼音和简写拼音、name_cut拼音和简写拼音
    spark.sql(
      s"""
         |insert overwrite table $database.car_4s_bssid_calculate_pinyin_base_info partition(day='$day')
         |select a.name,poi_lat,poi_lon,a.city_code,brand,ssid,device_num,connect_num,active_days,bssid_num,
    ...
    spark.sql(
      s"""
         |insert overwrite table $database.car_4s_bssid_calculate_pinyin_ssid_split_base_info partition(day='$day')
         |select name,poi_lat,poi_lon,city_code,brand,ssid,device_num,connect_num,active_days,bssid_num,
         ...
         |         regexp_replace(regexp_replace(regexp_replace(lower(ssid),'(\\\\s)',''),'([^a-z])','-'),
         |           'tp-link','') as ssid_en
         |  from $database.car_4s_bssid_calculate_pinyin_base_info
         |  where day='$day'
         |) t
    ...
    // 当用拼音全称来判断时，分段的ssid长度要>=4，4s店名字拼音必须包含分段的ssid，所在城市地区的拼音不能包含分段的ssid
    spark.sql(
      s"""
         |insert overwrite table $database.car_4s_split_ssid_pinyin_match partition(day='$day')
         |select name,poi_lat,poi_lon,city_code,brand,ssid,device_num,connect_num,active_days,bssid_num,
         |       pinyin_name,pinyin_city_area,ssid_en,ssid_split
         |from $database.car_4s_bssid_calculate_pinyin_ssid_split_base_info
         |where day='$day'
    ...
    // 当用拼音缩写来判断时，所在城市地区的拼音不能包含分段的ssid，分段的ssid长度要>=3并且4s店名字拼音必须包含分段的ssid
    spark.sql(
      s"""
         |insert overwrite table $database.car_4s_split_ssid_pinyin_short_match partition(day='$day')
         |select name,poi_lat,poi_lon,city_code,brand,ssid,device_num,connect_num,active_days,bssid_num,
         ...
         |and length(ssid_split)>=4
       """.stripMargin)

    // ssid不分段
    // 根据拼音全称来判断，4s店名字的拼音长度>=3并且4s店名字的拼音是否在整段的ssid内
    spark.sql(
      s"""
         |insert overwrite table $database.car_4s_ssid_pinyin_match partition(day='$day')
         |select name,poi_lat,poi_lon,city_code,brand,ssid,device_num,connect_num,active_days,bssid_num,
         ...
         |and length(pinyin_name_en)>=4
       """.stripMargin)

    // 根据拼音缩写来判断，4s店名字的拼音长度>=3并且4s店名字的拼音是否在整段的ssid内
    spark.sql(
      s"""
         |insert overwrite table $database.car_4s_ssid_pinyin_short_match partition(day='$day')
         |select name,poi_lat,poi_lon,city_code,brand,ssid,device_num,connect_num,active_days,bssid_num,
