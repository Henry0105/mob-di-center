...
object TrafficPinyinMatch {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val shoppingMallNameWord = spark.sql(
      s"""
         |select name
         |from $database.traffic_ssid_calculate_base_info
         |where day='$day'
    ...
    // join得到交通枢纽名字拼音和简写拼音、所在城市地区的拼音和简写拼音、name_cut拼音和简写拼音
    spark.sql(
      s"""
         |insert overwrite table $database.traffic_bssid_calculate_pinyin_base_info partition(day='$day')
         |select a.name,poi_lat,poi_lon,a.city_code,type_name,ssid,device_num,connect_num,active_days,bssid_num,flag,
    ...
    spark.sql(
      s"""
         |insert overwrite table $database.traffic_bssid_calculate_pinyin_ssid_split_base_info partition(day='$day')
         |select name,poi_lat,poi_lon,city_code,type_name,ssid,device_num,connect_num,active_days,bssid_num,flag,
         ...
         |  from $database.traffic_bssid_calculate_pinyin_base_info
         |  where day='$day'
         |) t
    ...
    // 当用拼音全称来判断时，分段的ssid长度要>=4，交通枢纽名字拼音必须包含分段的ssid，所在城市地区的拼音不能包含分段的ssid
    spark.sql(
      s"""
         |insert overwrite table $database.traffic_split_ssid_pinyin_match partition(day='$day')
         |select name,poi_lat,poi_lon,city_code,type_name,ssid,device_num,connect_num,active_days,bssid_num,flag,
         |       pinyin_name,pinyin_city_area,ssid_en,ssid_split
         |from $database.traffic_bssid_calculate_pinyin_ssid_split_base_info
         |where day='$day'
         |and instr(pinyin_name,ssid_split)>0
         |and instr(pinyin_city_area,ssid_split)=0
         |and ssid_split not in ('link')
         |and length(regexp_replace(ssid_split,'^[zcs]hu\\B|[aeoi]ng$$','0'))>=4
       """.stripMargin)

    // 当用拼音缩写来判断时，所在城市地区的拼音不能包含分段的ssid，分段的ssid长度要>=3并且交通枢纽名字拼音必须包含分段的ssid
    spark.sql(
      s"""
         |insert overwrite table $database.traffic_split_ssid_pinyin_short_match partition(day='$day')
         |select name,poi_lat,poi_lon,city_code,type_name,ssid,device_num,connect_num,active_days,bssid_num,flag,
    ...
    // ssid不分段
    // 根据拼音全称来判断，交通枢纽名字的拼音长度>=3并且交通枢纽名字的拼音是否在整段的ssid内
    spark.sql(
      s"""
         |insert overwrite table $database.traffic_ssid_pinyin_match partition(day='$day')
         |select name,poi_lat,poi_lon,city_code,type_name,ssid,device_num,connect_num,active_days,bssid_num,flag,
         |       pinyin_name,ssid_en
         |from
         |(
         ...
         |  where day='$day'
         |)a
         |where instr(ssid_en,pinyin_name)>0
         |and length(pinyin_name)>=3
       """.stripMargin)

    // 根据拼音缩写来判断，交通枢纽名字的拼音长度>=3并且交通枢纽名字的拼音是否在整段的ssid内
    spark.sql(
      s"""
         |insert overwrite table $database.traffic_ssid_pinyin_short_match partition(day='$day')
         |select name,poi_lat,poi_lon,city_code,type_name,ssid,device_num,connect_num,active_days,bssid_num,flag,
         ...
         |and length(pinyin_short_name)>=3
       """.stripMargin)
  }
}
