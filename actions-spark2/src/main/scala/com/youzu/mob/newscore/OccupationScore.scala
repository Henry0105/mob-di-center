...
object OccupationScore {
  ...
  def main(args: Array[String]): Unit = {
    ...
    data_training.select("device", "prediction", "probability")
      ...
      .repartition(10)
      .createOrReplaceTempView("lr_occupation_scoring")

    spark.sql(
      s"""
         |insert overwrite table $out_put_table partition (day = $day, kind = 'occupation')
         |select device, (prediction + 13) as prediction, probability
         |from lr_occupation_scoring
         |""".stripMargin)
