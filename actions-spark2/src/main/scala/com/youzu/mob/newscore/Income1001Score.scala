...
object Income1001Score {
  ...
  def main(args: Array[String]): Unit = {
    ...
    data_training.select("device", "prediction", "probability")
      ...
      .repartition(10)
      .createOrReplaceTempView("lr_income_1001_scoring")

    spark.sql(
      s"""
         | insert overwrite table $out_put_table partition(day = $day, kind='income_1001')
         | select device, (prediction+3) as prediction, probability
         | from lr_income_1001_scoring
      """.stripMargin)
  }
}
