...
object IosOfflineUniversalTools {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val dineinRows = dineinRdd.rdd.mapPartitions(row => {
      ...
      while (row.hasNext) {
        val r = row.next();
        val idfa: String = r.getAs[String]("idfa")
    ...
    val schemaString = "idfa,feature,cnt"
    dineinRows.mapPartitions(maps => {
      ...
      while (maps.hasNext) {
        ...
        for (f <- poiField) {
          ...
          if (!"".equals(value)) {
            rdds += ((idfa, feature, value))
          }
        }
        rdds += ((idfa, "total", "total=" + poi.map(p => {
          p.get("day").toString
        }).distinct.length))
      }
      rdds.toIterator
  ...
  def checkData(spark: SparkSession, day: String, windowTime: Int, lbstype: String): Boolean = {
    ...
    breakable {
      for (i <- 0 to (windowTime - 1)) {
        ...
        val count = fs.getContentSummary(new Path(s"/user/hive/warehouse/dm_mobdi_master.db/sdk_lbs_daily_poi_ios/type=${lbstype}/day=${bfday}"))
