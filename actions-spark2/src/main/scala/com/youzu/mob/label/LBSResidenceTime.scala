...
object LBSResidenceTime {
    ...
    val re = groupRows.mapPartitions(d => {
      ...
      while (d.hasNext) {
        ...
        while (rows.hasNext) {
          ...
          if (null == old) {
            old = r
          } else if ((!old._1.equals(lat) || !old._2.equals(lon)) && timeDiff(old._3, r._3) >= 60) {
            listRdd += ((device, old._1, old._2, old._3, r._3))
            old = r
          }
          if (!rows.hasNext) {
            listRdd += ((device, lat, lon, old._3, "23:59:59"))
          }
        }
      }
      listRdd.toIterator
  ...
  def timeDiff(beforeTime: String, afterTime: String): Long = {
    ...
    try {
      ...
      val before = sdf.parse(beforeTime).getTime
      val after = sdf.parse(afterTime).getTime
