...
object rta_device_update {
  ...
  def dw_gaasid_blacklist_table(spark: SparkSession, insert_day: String,full_par: String): Unit = {
    spark.sql(
      s"""
         ...
         |    select 'ieid' as type, ieid as idvalue
  ...
  def dw_gaasid_final_table(spark: SparkSession, insert_day: String): Unit = {
    spark.sql(
      s"""
         ...
         |if(oiid_black is null,oiid,'') as oiid from
         |(
         ...
         |    group by idvalue) b on a.ieid=b.ieid
         |    left join
         |    (select idvalue as oiid
         |    from $DW_GAASID_BLACKLIST
         |    where type='oiid' and day='$insert_day'
         |    group by idvalue) c on a.oiid=c.oiid
         |)d
         |""".stripMargin
    )
  }
  ...
  def dw_gaasid_in_installid_table(spark: SparkSession, insert_day: String): Unit = {
    spark.sql(
      s"""
         ...
         |        group by ieid,pkg) b on a.ieid=b.ieid
         ...
         |        group by oiid,pkg) b on a.oiid=b.oiid
         |    group by a.ieid,a.oiid,b.pkg
         |""".stripMargin
    )
  }
  ...
  def dw_device_with_cate_table(spark: SparkSession, insert_day: String,dim_app_pkg_par: String,app_category_par: String): Unit = {
    spark.sql(
      s"""
         ...
         |select ieid, oiid, c.apppkg, d.cate_l1, d.cate_l2
         |from
         ...
         |        group by pkg,apppkg) b on a.pkg=b.pkg) c
         |    left join
         |    (select apppkg, cate_l1, cate_l2
         |    from (select apppkg, cate_l1, cate_l2, row_number() over(partition by apppkg order by cate_l2) as num
         |        from
         |            (   select apppkg, cate_l1, cate_l2
  ...
  def dw_device_with_cate_installcount_table(spark: SparkSession, insert_day: String): Unit = {
    spark.sql(
      s"""
         ...
         |    select 'ieid' as idtype, ieid as idvalue, type, cate_l2_clean, count(1) as appnum
         |    from
         |        (select ieid, case when cate_l1_clean like '%游戏%' then 1 else 0 end as type, cate_l2_clean
         |        from $DW_DEVICE_WITH_CATE
         |        where day='$insert_day' and ieid is not null and trim(ieid)!='') a
         |    group by type,ieid,cate_l2_clean
         |union
         |    select 'oiid' as idtype, oiid as idvalue, type, cate_l2_clean, count(1) as appnum
         |    from
         |        (select oiid, case when cate_l1_clean like '%游戏%' then 1 else 0 end as type, cate_l2_clean
         |        from $DW_DEVICE_WITH_CATE
         |        where day='$insert_day' and oiid is not null and trim(oiid)!='') a
  ...
  def dm_device_rec_for_0324_pre_table(spark: SparkSession, insert_day: String): Unit = {
    spark.sql(
      s"""
         ...
         |    select '0324' as code, idtype, idvalue, 0.8 as recommend, 1 as status
         |    from $DW_DEVICE_WITH_CATE_INSTALLCOUNT
         |    where day='$insert_day' and type=1
         |    group by idtype,idvalue
         |union
         |    select '0324' as code, a.idtype, a.idvalue, 0.1 as recommend, 1 as status
         |    from (select idtype, idvalue
         |        from $DW_DEVICE_WITH_CATE_INSTALLCOUNT
         |        where day='$insert_day' and type!=1
         |        group by idtype,idvalue) a
         |        left join (select idtype, idvalue
         |        from $DW_DEVICE_WITH_CATE_INSTALLCOUNT
         |        where day='$insert_day' and type=1
         |        group by idtype,idvalue) b on a.idtype=b.idtype and a.idvalue=b.idvalue
  ...
  def dm_device_rec_for_0324_pre_second_table(spark: SparkSession, insert_day: String,full_par: String): Unit = {
    spark.sql(
      s"""
         create table $DM_DEVICE_REC_FOR_0324_PRE_SECOND
         as
             select code,idtype,idvalue,recommend,status
             from $DM_DEVICE_REC_FOR_0324_PRE
         union
             select a.code,a.idtype,a.idvalue,a.recommend,1 as status
             from (select code,idtype,idvalue,recommend
             from $DM_DEVICE_REC_FOR_0324
             where day='$full_par' and status!=0) a
             left  join $DM_DEVICE_REC_FOR_0324_PRE b on a.idtype=b.idtype and a.idvalue=b.idvalue where b.idvalue is null
             left  join
             (select idtype,idvalue
             from $DW_DEVICE_WITH_CATE_INSTALLCOUNT
             where day='$insert_day' group by idtype,idvalue) c on a.idtype=c.idtype and a.idvalue=c.idvalue where c.idvalue is null
             left  join
             (select type as idtype,idvalue
             from $DW_GAASID_BLACKLIST where day='$insert_day') d on a.idtype=d.idtype and a.idvalue=d.idvalue where d.idvalue is null
         |""".stripMargin
    )
  }
  ...
  def dm_device_rec_for_0325_pre_table(spark: SparkSession): Unit = {
      spark.sql(
        s"""
           ...
           |    select '03251' as code,'ieid' as idtype,id as idvalue,recommend,1 as stauts from (select
                ...
                from (select distinct(id) from dw_dpi_feedback.ods_dpi_mkt_feedback_incr
                where  load_day>='20220328' and source='guangdong_mobile' and model_type='timewindow' and tag!='mb11') a
                left join
                (select a.id,count(1) as cnt
                from (select id
                from dw_dpi_feedback.ods_dpi_mkt_feedback_incr
                where datediff(current_date,date(concat(substr(day,1,4),'-',substr(day,5,2),'-',substr(day,-2))))<=14
                and source='guangdong_mobile' and model_type='timewindow' and tag!='mb11' group by id,day) a group by id) b on a.id=b.id) c
            union
                select '03251' as code,'oiid' as idtype,d.oiid as idvalue,recommend,1 as status from (select
                a.id,case when cnt is null then 1 when cnt=1 then 1.01 when cnt=2 then 1.02 when cnt=3 then 1.03
                when cnt=4 then 1.04 when cnt=5 then 1.05 when cnt=6 then 1.06 when cnt=7 then 1.07
                when cnt=8 then 1.08 when cnt=9 then 1.09 when cnt=10 then 1.1 when cnt=11 then 1.11
                when cnt=12 then 1.12 when cnt=13 then 1.13 when cnt=14 then 1.14
                end as recommend
                from (select distinct(id) from $ODS_DPI_MKT_FEEDBACK_INCR
                where  load_day>='20220328' and source='guangdong_mobile' and model_type='timewindow' and tag!='mb11') a
                left join
                (select a.id,count(1) as cnt
                from (select id
                ...
                where datediff(current_date,date(concat(substr(day,1,4),'-',substr(day,5,2),'-',substr(day,-2))))<=14
                and source='guangdong_mobile' and model_type='timewindow' and tag!='mb11' group by id,day) a group by id) b on a.id=b.id) c
                join $DPI_IEID_OIID_20220329 d on c.id=d.ieid
            union
                select '03252' as code,'ieid' as idtype,a.id as idvalue,
                case when diffdays<=7 then 0.9 when diffdays>7 and diffdays<=14 then 1 when diffdays>14 then 1.1 end as recommend,1 as status
                from
                ...
                from $ODS_DPI_MKT_FEEDBACK_INCR
                where load_day>='20220328' and source='guangdong_mobile' and model_type='timewindow' and tag!='mb11' group by id) a
            union
                select '03252' as code,'oiid' as idtype,b.oiid as idvalue,
                case when diffdays<=7 then 0.9 when diffdays>7 and diffdays<=14 then 1 when diffdays>14 then 1.1 end as recommend,1 as status
                from
                (select id,cast(datediff(current_date,date(concat(substr(max(day),1,4),'-',substr(max(day),5,2),'-',substr(max(day),-2)))) as int) as diffdays
                from $ODS_DPI_MKT_FEEDBACK_INCR
                where load_day>='20220328' and source='guangdong_mobile' and model_type='timewindow' and tag!='mb11' group by id) a
                join $DPI_IEID_OIID_20220329 b on a.id=b.ieid
           |""".stripMargin)
    }
    ...
    spark.sql(
      s"""
         ...
         |     select code,idtype,idvalue,
               ...
               from (
                select a.idtype,a.idvalue,a.appnum,b.code_id as code from (select
                idtype,idvalue,appnum,cate_l2_clean from $DW_DEVICE_WITH_CATE_INSTALLCOUNT where day='$insert_day' and type=1) a
               join (select  game_cat,code_id from sdk_test.rta_gameapp_cate_40) b
  ...
  def dm_device_rec_for_0326_pre_second_table(spark: SparkSession, insert_day: String,full_par: String): Unit = {
      spark.sql(
        s"""
           create table DM_DEVICE_REC_FOR_0326_PRE_SECOND
           as
               select code,idtype,idvalue,recommend,status
           ...
           union
               ...
               where day='$full_par' and status!=0) a
               left  join $DM_DEVICE_REC_FOR_0326_PRE b on a.idtype=b.idtype and a.idvalue=b.idvalue where b.idvalue is null
               left  join
               (select idtype,idvalue
               from $DW_DEVICE_WITH_CATE_INSTALLCOUNT
               where day='$insert_day' group by idtype,idvalue) c on a.idtype=c.idtype and a.idvalue=c.idvalue where c.idvalue is null
               left  join
               (select type as idtype,idvalue
               from $DW_GAASID_BLACKLIST where day='$insert_day') d on a.idtype=d.idtype and a.idvalue=d.idvalue where d.idvaule is null
           |""".stripMargin
      )
    }
  ...
  def dm_device_rec_for_0327_pre_table(spark: SparkSession): Unit = {
    spark.sql(
      s"""
         ...
         |        join $DW_GAASID_DOUBTFUL b on a.type = b.type
         |            and a.idvalue = b.idvalue
         |union
         ...
         |        join $DW_GAASID_BLACKLIST b on a.type = b.type
         |            and a.idvalue = b.idvalue
         |""".stripMargin
    )
  }
