...
object WorkAndLivePlaceClusterLive extends SparkHelper {
    ...
    val generateSQLs = stage.substring(0, 1) match {
      ...
      case "B" =>
        ...
        val joinWorkSql =
          s"""
             |left join
             |(
             |  select device from $DM_MOBDI_TMP.new_live_tmp_device_work_place where stage='A' group by device
             |)t1
             |on tmp_cluster_work_live.device = t1.device
           """.stripMargin
      ...
      case "C" =>
        ...
        val joinLiveSql =
          s"""
             |left join
             |(
             |  select device from $DM_MOBDI_TMP.new_live_tmp_device_live_place where stage='A' group by device
             |)t1
             |on tmp_cluster_work_live.device  = t1.device
             |left join
             |(
             |  select device from $DM_MOBDI_TMP.new_live_tmp_device_live_place where stage='B' group by device
             |)t2
        ...
        val joinWorkSql =
          s"""
             ...
             |left join
             |(
             |  select device from $DM_MOBDI_TMP.new_live_tmp_device_work_place where stage='B' group by device
             |)t2
             |on tmp_cluster_work_live.device=t2.device
           """.stripMargin
        (generateConfidenceSQL(stage, confidenceLive, joinLiveSql, "live"),
      ...
      case "D" =>
       ...
       """.stripMargin)
        ...
        val work =
          s"""
             |insert overwrite  table $DM_MOBDI_TMP.new_live_tmp_device_work_place partition(stage='$stage')
             |select
             |   device,
             |   '' as cluster,
             |   lon as centerlon,
        ...
        val live =
          s"""
             |insert overwrite  table $DM_MOBDI_TMP.new_live_tmp_device_live_place partition(stage='$stage')
             |select
             |   device,
             |   '' as cluster,
             |   lon as centerlon,
  ...
  private def preCluster(prepareSql: String, clusterTable: String,
    ...
    spark.sql(
      s"""
         |insert overwrite table $DM_MOBDI_TMP.new_live_tmp_device_dbscan_result partition(stage='$clusterTable')
         |select * from tmp_rs_dbscan
       """.stripMargin)
    spark.sql(
      s"""
         |select * from  $DM_MOBDI_TMP.new_live_tmp_device_dbscan_result where stage='$clusterTable'
    ...
    spark.sql(
      s"""
         |insert overwrite table $DM_MOBDI_TMP.new_live_tmp_device_location_cluster_rank partition(stage='$clusterTable')
         |select
         |   device,
         |   cluster,
         |   centerlon,
    ...
    spark.sql(
      s"""
         ...
         |        from
         |        $DM_MOBDI_TMP.new_live_tmp_device_location_cluster_rank
  ...
  private def generateConfidenceSQL(clusterTable: String, confidence: String, joinSql: String, name: String) = {
    ...
    s"""
       |insert overwrite table $DM_MOBDI_TMP.new_live_tmp_device_${name}_place partition(stage='$clusterTable')
       |select
