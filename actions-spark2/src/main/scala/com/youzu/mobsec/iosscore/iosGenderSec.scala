...
class iosGenderSec(@transient spark: SparkSession) {
  ...
  """.stripMargin)
    ...
    val joinData2 = rdd_score_1.join(rdd_train_2, Seq("ifid"), "inner")
    ...
    predictions_lr.select("ifid","prediction","probability").map(r => (r.getString(0),r.getDouble(1),r.getAs[org.apache.spark.ml.linalg.Vector](2).apply(r.getDouble(1).toInt))).toDF("ifid","prediction","probability").createOrReplaceTempView("tmpTable2")
  ...
  def chisq(args: Array[String]): Unit = {
    ...
    var rdd_training = spark.sql(
      s"""
         |   select ifid,cast(gender as double) as tag, index, cnt
         |   from $beforechi
  """.stripMargin)
...
object iosGenderSec {
  def main(args: Array[String]): Unit = {
    ...
    new iosGenderSec(spark).score(args)
  }
}

object iosGenderSecChisq {
  def main(args: Array[String]): Unit = {
    var day = args(0)
    ...
    new iosGenderSec(spark).chisq(args)
  }
}