...
with ids as (  
select  device 
from mobdi_muid_dashboard.duid_dau_other_id_percent_statement_by_dws_device_sdk_run_master_di 
where day='${day}' and plat=1 and
duid_type in ('oiid_in_duid','ieid_in_duid','isid_in_duid','pid_in_duid_android') 
group by device 
),
applist as ( 
select device 
...
from mobdi_muid_dashboard.duid_dau_other_id_percent_statement_by_dws_device_sdk_run_master_di   
where day='${day}' and plat=1 and
duid_type ='gps_base_wifi_in_duid_android' 
group by device
) 


insert overwrite table  mobdi_muid_dashboard.duid_dau_report partition(day=$day)
