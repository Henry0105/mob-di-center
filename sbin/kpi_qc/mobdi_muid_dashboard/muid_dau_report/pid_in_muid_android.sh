...
insert overwrite table mobdi_muid_dashboard.muid_dau_other_id_percent_statement_by_dws_device_sdk_run_master_di partition(day=$day,muid_type='pid_in_muid_android')
...
(
select device
from dm_mobdi_topic.dws_device_pid_di
where day=$day
group by device
) as b 
