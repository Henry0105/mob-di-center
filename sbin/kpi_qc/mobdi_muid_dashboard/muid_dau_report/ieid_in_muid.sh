...
insert overwrite table mobdi_muid_dashboard.muid_dau_other_id_percent_statement_by_dws_device_sdk_run_master_di partition(day=$day,muid_type='ieid_in_muid')
...
(
select device
from dm_mobdi_topic.dws_device_ieid_di
where day = $day
group by device
) as b 
