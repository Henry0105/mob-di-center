...
drop table if exists mobdi_muid_dashboard.muid_mau_android_by_dws_device_sdk_run_master_di_pre;
create table mobdi_muid_dashboard.muid_mau_android_by_dws_device_sdk_run_master_di_pre stored as orc as 
select device
from 
(
     select muid as device 
     from dm_mobdi_master.dwd_log_run_new_di
     where day <= '$day' and day >= '$firstDayOfMonth' and plat = 1
     group by muid
     union all
     select muid as device
