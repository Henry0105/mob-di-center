...
:<<!
create table if not exists mobdi_muid_dashboard.pid_in_muid_mau_android_by_dws_device_sdk_run_master_di(
pid_mau_his bigint comment'月活muid中有pid的量(历史采集到过pid就算)',
pid_mau_now bigint comment'月活muid中有pid的量(当月采集到的才算)'
)comment 'android版本月活muid中有pid的量'
...
insert overwrite table mobdi_muid_dashboard.pid_in_muid_mau_android_by_dws_device_sdk_run_master_di partition(day='$day')
