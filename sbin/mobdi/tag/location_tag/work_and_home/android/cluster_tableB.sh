...
insert overwrite table $tmp_device_location_stage_pre partition(stage='B')
...
(
   ...
   from
   (
      select device from $tmp_device_live_place  where stage='A' and confidence <> 0.5 group by device
   ) liveplace
  inner join
   (
      select device from $tmp_device_work_place  where stage='A' and confidence <> 0.5 group by device
   ) workplace
...
prepare_sql="
select * from $tmp_device_location_stage_pre where stage ='B'
"
...
spark2-submit --master yarn \
        ...
        /home/dba/mobdi_center/lib/MobDI-spark2-1.0-SNAPSHOT-jar-with-dependencies.jar "B" "${prepare_sql}" 1
