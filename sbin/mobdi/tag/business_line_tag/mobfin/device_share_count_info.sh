...
## 目标表
#label_l1_anticheat_device_share_wi=dm_mobdi_report.label_l1_anticheat_device_share_wi
...
with sharecount_1week as(
  select deviceid as device, count(*) as cnt 
  from 
  (
    select deviceid, snsplat, clienttime, apppkg, content, appver
    from $dwd_log_share_new_di
    where day > '$p1week' and day <= '$day'
    and plat = 1 and snsplat <= 65 and snsplat is not null
    group by deviceid, snsplat, clienttime, apppkg, content, appver
    having count(*) = 1
  ) as a 
  group by deviceid
),
sharecount_2weeks  as(
  select deviceid as device, count(*) as cnt 
  from 
  (
    select deviceid, snsplat, clienttime, apppkg, content, appver
...
insert overwrite table $label_l1_anticheat_device_share_wi partition(day='$day')
select
...
from
 (select device, cnt from sharecount_1week) a
 full join
 ( select
    ...
    from
    (select device, cnt from sharecount_1month) t1
    full join
    (select device, cnt from sharecount_2weeks) t2
