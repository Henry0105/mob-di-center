...
insert overwrite table $label_l1_anticheat_device_cnt partition(day = '$day',timewindow = '$timewindow',flag = '3')
select device,count(1) as cnt
from $tmp_anticheat_device_gps_ip_location
where day = '$day'
and timewindow = '$timewindow'
and province_code_ip <> province_code_gps
group by device;
" 
}
