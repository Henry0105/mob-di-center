...
insert overwrite table $tmp_anticheat_device_unstall_7days_pre partition (day = '$day')
...
insert overwrite table $tmp_anticheat_device_unstall_14days_pre partition (day = '$day')
select device,cnt_pkg,cnt_all 
from 
(
    select device,count(1) over(partition by device) as cnt_pkg,sum(cnt) over(partition by device) as cnt_all
...
insert overwrite table $tmp_anticheat_device_unstall_30days_pre partition (day = '$day')
select device,cnt_pkg,cnt_all 
from 
(
    select device,count(1) over(partition by device) as cnt_pkg,sum(cnt) over(partition by device) as cnt_all
