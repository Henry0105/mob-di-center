...
## 目标表
#label_l1_anticheat_mcid_sec_mf=dm_mobdi_report.label_l1_anticheat_mcid_sec_mf

mcid_mapping_partition_sql="
add jar hdfs://ShareSdkHadoop/dmgroup/dba/commmon/udf/udf-manager-0.0.7-SNAPSHOT-jar-with-dependencies.jar;
create temporary function GET_LAST_PARTITION as 'com.youzu.mob.java.udf.LatestPartition';
SELECT GET_LAST_PARTITION('dm_dataengine_mapping', 'dm_mcid_mapping', 'day');
drop temporary function GET_LAST_PARTITION;
"
mcid_mapping_last_day=(`hive -e "$mcid_mapping_partition_sql"`)
...
set mapred.job.name=mcid_count_info_sec;

insert overwrite table $label_l1_anticheat_mcid_sec_mf partition(day='$day')
select
  other_info.mcid, ieid_cn, snid_cn, nvl(model_cn, 0), isid_cn, pid_cn
from
( select
    mcid,
  ...
  from $dm_mcid_mapping_v3_sec
  where day='$mcid_mapping_last_day'
) other_info
left join
( select
    mcid, count(1) as model_cn
  from
  ( select
      mcid, coalesce(model, '') as model, coalesce(factory, '') as factory
    from  ( select
              mcid, d as device
            from $dm_mcid_mapping_v3_sec
            lateral view explode(device) devices as d
            where day='$mcid_mapping_last_day' and device is not null
          ) mcid_info
          left join ( select
    ...
    group by mcid, coalesce(model, ''), coalesce(factory, '')
  ) t
  where model != '' or factory != ''
  group by mcid
) model_info
on other_info.mcid = model_info.mcid
;
"
