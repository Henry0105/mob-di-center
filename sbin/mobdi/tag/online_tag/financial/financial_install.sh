...
@SourceTable:$app_tag_system_mapping_par,$tag_cat_mapping_dmp_par,$dws_device_install_app_re_status_di,$app_pkg_mapping_par
@TargetTable:$dim_online_category_mapping
@TableRelation:$app_tag_system_mapping_par,$tag_cat_mapping_dmp_par->$dim_online_category_mapping|$dim_online_category_mapping,$app_pkg_mapping_par,$dws_device_install_app_re_status_di->rp_mobdi_app.timewindow_online_profile
'
day=$1
mappingtype=14
computeType=0
...
SELECT a.apppkg AS relation,
 ...
 FROM (select rank.apppkg as apppkg, 
       ...
       from $app_tag_system_mapping_par n where version ='1000') rank where rank.rank =1
) a 
JOIN  (select cat2,tag from $tag_cat_mapping_dmp_par where version='1000')b
ON a.tag = b.tag
...
from (select n.apppkg as apppkg,n.tag as tag ,n.norm_tfidf as norm_tfidf ,
Row_number() over(partition by n.apppkg,n.tag ORDER BY  n.norm_tfidf DESC ) as rank 
from $app_tag_system_mapping_par n where version ='1000'  ) rank where rank.rank =1) a
