...
:<<!
@part_1:
实现功能: 对device进行car的模型计算
实现步骤: 1.使用逻辑回归对数据进行模型训练, 结果输出到kind='car'分区
!
...
model="/dmgroup/dba/modelpath/20190815/linear_regression_model/carmodel"
length=330000
...
(
  ...
  inner join
  $model_index t on version = '1003' and model = 'car' and t.index=t1.index
...
/home/dba/mobdi_center/lib/MobDI-center-spark2-1.0-SNAPSHOT-jar-with-dependencies.jar "$model" "$pre_sql" "$length" "$outputTable" "$day"
