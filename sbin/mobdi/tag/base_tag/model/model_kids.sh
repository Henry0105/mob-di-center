...
modelPath="/dmgroup/dba/modelpath/20190815/linear_regression_model/kidsmodel"
length=330000
...
(
  ...
  (
    ...
    inner join
    $model_index t on version = '1003' and model = 'kids' and t.index=t1.index
    group by device, t1.index, cnt
  ) t2
  group by device
) as a1
left join
(select * from $label_kids_score_di where day=$day ) a2 on a1.device = a2.device
"

spark2-submit --master yarn --deploy-mode cluster \
--class com.youzu.mob.newscore.KidsScore \
--driver-memory 8G \
--executor-memory 15G \
--executor-cores 5 \
...
/home/dba/mobdi_center/lib/MobDI-center-spark2-1.0-SNAPSHOT-jar-with-dependencies.jar "$modelPath" "$tmp_sql" "$length" "$outputTable" "$day"
