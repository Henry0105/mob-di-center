...
group by device,bssid,plat,from_unixtime(cast(substring(datetime, 1, 10) as bigint), 'yyyyMMdd HH:mm:ss');
"
...
set hive.merge.smallfiles.avgsize=250000000;
set hive.merge.size.per.task = 250000000;
insert overwrite table $income_1001_pid_contacts_index partition(day='$day')
...
inner join
$pid_contacts_index d on d.day='$pidContactsIndexLastPar' and c.pid=d.pid;

insert overwrite table $income_1001_pid_contacts_word2vec_index partition(day='$day')
select device,index
from
(
  select a.device,
  ...
  inner join
  $pid_contacts_word2vec_index b on b.day='$pidContactsWord2vecIndexLastPar' and a.pid=b.pid
  where a.day='$day'
) t
