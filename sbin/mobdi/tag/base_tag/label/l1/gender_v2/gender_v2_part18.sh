...
with gender_app2vec_device_tgi_vec2_score as (
...
join (select apppkg, tgi_level from $dim_gender_app_tgi_level_5 group by apppkg, tgi_level) z on x.apppkg=z.apppkg
group by device, tgi_level
)
insert overwrite table $gender_feature_v2_part18 partition(day=$day)
select device, 
...
from (
  ...
  from (
    ...
    from (
      ...
      join $dim_gender_app2vec_tgi5_center2 bb on aa.tgi_level=bb.tgi_level
    ...
    (
      ...
      from gender_app2vec_device_tgi_vec2_score group by device, tgi_level
