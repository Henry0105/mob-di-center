...
day_before_three_month=$(date -d "${day} -3 month" "+%Y%m%d")
...
age_new_active_days_3="${dm_mobdi_tmp}.age_new_active_days_3"
...
insert overwrite table $age_new_active_days_3 partition (day='$insertday')
select device
,max(case when cate_id='tgi2_55_0' then cnt_1 else 0 end ) as tgi2_55_0_act_day_3m
,max(case when cate_id='tgi2_18_24_0' then cnt_1 else 0 end ) as tgi2_18_24_0_act_day_3m
from
(
    ...
    (
        ...
        from(
                ...
                from(
                    ...
                    where day between $day_before_three_month and $day
...
#hive -e "
#alter table $age_new_active_days_3 drop partition(day< $day_before_one_month);
#"