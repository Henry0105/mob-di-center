...
SET mapreduce.reduce.java.opts='-Xmx11g';
...
insert overwrite table dm_mobdi_mapping.android_id_mapping_full partition (version='${day}.1000')
select
...
from (
  select * from dm_mobdi_mapping.android_id_mapping_full where version = '$full_last_version'
  and device is not null and length(device)= 40 and device = regexp_extract(device,'([a-f0-9]{40})', 0)
) a
full join
(select * from dw_mobdi_md.android_id_mapping_incr where day='$day') b
...
if [[ "$delete_day" -ne "$LastMonthLastDay" ]]; then
  ...
  hive -e "alter table dm_mobdi_mapping.android_id_mapping_full  DROP IF EXISTS PARTITION (version='${delete_day}.1000');"
  echo "deleting version: ${delete_day}.1001 if exists"
  hive -e "alter table  dm_mobdi_mapping.android_id_mapping_full  DROP IF EXISTS PARTITION (version='${delete_day}.1001');"
  echo "deleting version: ${delete_day}.1002 if exists"
  hive -e "alter table  dm_mobdi_mapping.android_id_mapping_full  DROP IF EXISTS PARTITION (version='${delete_day}.1002');"
fi
# ### END DELETE
