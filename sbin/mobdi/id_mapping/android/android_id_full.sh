...
insert overwrite table $dim_id_mapping_android_df partition (version='${day}.1000')
select
...
if [[ "$delete_day" -ne "$LastMonthLastDay" ]]; then
  ...
  hive -e "alter table $dim_id_mapping_android_df  DROP IF EXISTS PARTITION (version='${delete_day}.1000');"
  echo "deleting version: ${delete_day}.1001 if exists"
  hive -e "alter table  $dim_id_mapping_android_df  DROP IF EXISTS PARTITION (version='${delete_day}.1001');"
  echo "deleting version: ${delete_day}.1002 if exists"
  hive -e "alter table  $dim_id_mapping_android_df  DROP IF EXISTS PARTITION (version='${delete_day}.1002');"
fi
# ### END DELETE
