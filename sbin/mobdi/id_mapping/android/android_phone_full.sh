...
if [[ "$delete_day" -ne "$LastMonthLastDay" ]]; then
  ...
  hive -e "alter table $dim_phone_mapping_df  DROP IF EXISTS PARTITION (version='${delete_day}.1000',plat=1);"
  echo "deleting version: ${delete_day}.1001 if exists"
  hive -e "alter table  $dim_phone_mapping_df  DROP IF EXISTS PARTITION (version='${delete_day}.1001',plat=1);"
  echo "deleting version: ${delete_day}.1002 if exists"
  hive -e "alter table  $dim_phone_mapping_df  DROP IF EXISTS PARTITION (version='${delete_day}.1002',plat=1);"
fi
# ### END DELETE
