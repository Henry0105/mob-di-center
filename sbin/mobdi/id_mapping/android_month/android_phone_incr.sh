...
select
  device,
  concat_ws(',',if(length(trim(phone_mac))=0,null,phone_mac),if(length(trim(phone_imei))=0,null,phone_imei)) as phone,
...
insert overwrite table $dws_phone_mapping_di partition(day=$enddate,plat=1)
