...
android_id_mapping_incr=dm_mobdi_topic.dws_id_mapping_android_di
...
with android_device_carrierarray as (  --device对应的 carrier array
  ...
  (
    ...
    where day='20151231' and size(ARRAY_DISTINCT(split(imsi, ','), imsiarray)) > 0
  ) t1
...
insert overwrite table $android_id_mapping_incr partition(day='20151231')
select
