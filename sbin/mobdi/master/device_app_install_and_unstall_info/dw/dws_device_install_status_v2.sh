...
:<<!
create table dm_mobdi_topic.dws_device_install_status (
    device string comment '设备主键',
    pkg string comment '安装包主键',
    install_datetime string comment '最近一次安装时间',
    unstall_datetime string comment '最近一次卸载时间',
    all_datetime string comment '设备对应安装包最近一次上传全量时间',
    df_final_time string comment '设备最近一次上传全量时间',
    final_flag string comment '作为最主要的标识pkg的状态的标志，-1代表卸载，0代表在装列表出现，1代表新安装',    
    final_time string comment '最后一次的动作时间',
    reserved_flag string comment '用来标识新安装的flag，来产生master_reserved_new，1代表新安装，0代表在装，-1代表卸载',
    process_time string comment '数据处理时间'
)
partitioned by (day string comment '日期')
stored as orc;

create table dw_mobdi_md.tmp_device_install_status (
    device string comment '设备主键',
    pkg string comment '安装包主键',
    install_datetime string comment '最近一次安装时间',
    unstall_datetime string comment '最近一次卸载时间',
...
insert overwrite table dw_mobdi_md.tmp_device_install_status partition (day = '$day')
select device, 
       pkg, 
       install_datetime, 
       unstall_datetime, 
...
(
    select *
    from dw_mobdi_md.tmp_device_install_status
    where day = '$day'
) as a 
left join 
(
  select device, max(day) as process_time
  from 
  (
    select trim(lower(muid)) as device, day 
