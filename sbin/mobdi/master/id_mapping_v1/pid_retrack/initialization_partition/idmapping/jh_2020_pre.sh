...
drop table if exists mobdi_test.gai_device_phone_imei_mac_2020_pre;
create table mobdi_test.gai_device_phone_imei_mac_2020_pre as
select device, dt,
...
(
  ...
  where jh.dt < '20200101'  and jh.plat = 1 and muid is not null and length(muid) = 40 and muid = regexp_extract(muid, '([a-f0-9]{40})', 0)
  ...
  (
    ...
    where dt < '20200101' and plat = 1 and muid is not null and length(muid) = 40 and muid = regexp_extract(muid, '([a-f0-9]{40})', 0)
    group by muid, dt
  ) as jh
  on info.muid = jh.device and info.dt = jh.dt
  where info.dt < '20200101' and info.plat = 1 and muid is not null and length(muid) = 40 and jh.device is null
) as tt
