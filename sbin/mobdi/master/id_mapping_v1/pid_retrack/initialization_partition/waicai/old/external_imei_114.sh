...
drop table if exists mobdi_test.gai_external_imei_114;
create table mobdi_test.gai_external_imei_114 stored as orc as
...
(
  ...
  where cast(conv(substring(device,0,1), 16, 10) as int)=14
) as a 
inner join 
(
