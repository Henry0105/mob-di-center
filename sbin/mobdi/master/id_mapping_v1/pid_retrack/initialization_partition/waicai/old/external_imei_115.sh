...
drop table if exists mobdi_test.gai_external_imei_115;
create table mobdi_test.gai_external_imei_115 stored as orc as
...
(
  ...
  where cast(conv(substring(device,0,1), 16, 10) as int)=15
) as a 
inner join 
(
  select lower(trim(owner_data)) as imei, 
  ...
  where type in ('imei_phone', 'imei14_phone', 'imei15_phone')
  group by lower(trim(owner_data)),split(EXTRACT_PHONE_NUM(trim(ext_data)), ',')[0],cast(unix_timestamp(processtime, 'yyyyMMdd') as string)
) as b 
