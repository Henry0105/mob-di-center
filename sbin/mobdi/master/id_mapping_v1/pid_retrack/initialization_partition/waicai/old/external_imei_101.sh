...
drop table if exists mobdi_test.gai_external_imei_101;
create table mobdi_test.gai_external_imei_101 stored as orc as
...
(
  ...
  where cast(conv(substring(device,0,1), 16, 10) as int)=1
) as a 
inner join 
(
