...
drop table if exists mobdi_test.gai_external_imei_100;
create table mobdi_test.gai_external_imei_100 stored as orc as
...
(
  ...
  where cast(conv(substring(device,0,1), 16, 10) as int)=0
) as a 
inner join 
(
