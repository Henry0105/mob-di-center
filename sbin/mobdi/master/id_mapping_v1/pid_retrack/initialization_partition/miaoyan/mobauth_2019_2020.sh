...
drop table if exists mobdi_test.gai_device_phone_2020_pre;
create table mobdi_test.gai_device_phone_2020_pre stored as orc as
...
(
  ...
  where day <'20210101' and plat = 1 and muid is not null
  and length(muid) = 40
  and muid = regexp_extract(muid, '[0-9a-f]{40}', 0)
