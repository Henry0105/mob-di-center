...
SELECT GET_LAST_PARTITION('ex_log', 'dim_device_serialno_merge_df', 'day');
...
insert overwrite table $dim_device_serialno_merge_df partition (day='$insert_day')
select
  device,
  serialno,
  serialno_tm,
...
where serialno is not null and length(serialno)>0
"
...
if [[ "$delete_day" -ne "$LastMonthLastDay" ]]; then
  ...
  hive -e "alter table $dim_device_serialno_merge_df DROP IF EXISTS PARTITION (day='${delete_day}');"
fi
# ### END DELETE