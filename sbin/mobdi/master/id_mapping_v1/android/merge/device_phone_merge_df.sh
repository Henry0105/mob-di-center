...
SELECT GET_LAST_PARTITION('ex_log', 'dim_device_phone_merge_df', 'day');
...
insert overwrite table $dim_device_phone_merge_df partition (day='$insert_day')
select
  device,
  phone,
  phone_tm,
...
function qc_id_mapping(){
  cd `dirname $0`
  sh /home/dba/mobdi_center/qc/real_time_mobdi_qc/qc_id_mapping_view_merge.sh  "$dim_device_phone_merge_df" "${insert_day}" "$last_par" || qc_fail_flag=1
...
if [[ "$delete_day" -ne "$LastMonthLastDay" ]]; then
  ...
  hive -e "alter table $dim_device_phone_merge_df DROP IF EXISTS PARTITION (day='${delete_day}');"
fi
# ### END DELETE