...
insert overwrite table $dws_device_pidno_di partition(day='$insert_day')
select
  device,
...
# 分区清理，保留最近14个分区
for old_version in `hive -e "show partitions $dws_device_pidno_di" | grep -v '_bak' | sort | head -n -14`
do
  echo "rm $old_version"
  hive -e "alter table $dws_device_pidno_di drop if exists partition ($old_version)"
done
