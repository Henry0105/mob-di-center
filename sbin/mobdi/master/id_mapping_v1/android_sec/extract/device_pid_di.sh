...
dwd_log_device_pid_sec_di=dm_mobdi_master.dwd_log_device_pid_sec_di
...
insert overwrite table $dws_device_pid_di partition(day='$insert_day')
select
  device,
...
(
    ...
    where day='$insert_day' and plat = 1 and muid is not null and length(trim(muid))=40 and pid is not null and length(pid)>0
    ...
    (
        ...
        from $dwd_log_device_pid_sec_di
    ...
    select device,
           pid,
           cast(max(datetime) as string) as pid_tm
    from
    (
        select muid as device,
               pid,
...
# 分区清理，保留最近14个分区
for old_version in `hive -e "show partitions $dws_device_pid_di" | grep -v '_bak' | sort | head -n -14`
do
  echo "rm $old_version"
  hive -e "alter table $dws_device_pid_di drop if exists partition ($old_version)"
done
