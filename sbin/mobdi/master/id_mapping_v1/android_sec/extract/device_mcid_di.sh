...
set mapreduce.map.java.opts='-Xmx6554m' -XX:+UseG1GC;
set mapreduce.child.map.java.opts='-Xmx6554m';
...
# 分区清理，保留最近14个分区
for old_version in `hive -e "show partitions $dws_device_mcid_di" | grep -v '_bak' | sort | head -n -14`
do
  echo "rm $old_version"
