...
# output
dim_device_mobauth_pid_merge_df=dim_mobdi_mapping.dim_device_mobauth_pid_merge_df


last_par_sql="
add jar hdfs://ShareSdkHadoop/dmgroup/dba/commmon/udf/udf-manager-0.0.7-SNAPSHOT-jar-with-dependencies.jar;
create temporary function GET_LAST_PARTITION as 'com.youzu.mob.java.udf.LatestPartition';
SELECT GET_LAST_PARTITION('dim_mobdi_mapping', 'dim_device_mobauth_pid_merge_df', 'day');
...
insert overwrite table $dim_device_mobauth_pid_merge_df partition (day='$insert_day')
select
  device,
  pid,
  pid_tm,
...
(
    select *
    from $dim_device_mobauth_pid_merge_df
    where day = '$last_par'
...
(
    select *
    from $dws_device_mobauth_pid_di
...
function qc_id_mapping(){
  cd `dirname $0`
  sh /home/dba/mobdi_center/qc/real_time_mobdi_qc/qc_id_mapping_view_merge.sh  "$dim_device_mobauth_pid_merge_df" "${insert_day}" "$last_par" || qc_fail_flag=1
...
if [[ "$delete_day" -ne "$LastMonthLastDay" ]]; then
  ...
  hive -e "alter table $dim_device_mobauth_pid_merge_df DROP IF EXISTS PARTITION (day='${delete_day}');"
fi
# ### END DELETE