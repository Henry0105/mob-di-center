...
/home/dba/lib/MobDI-spark-1.0-SNAPSHOT_lbs.jar "${day}" "${beforeNum}" "${lbs_tmp}" "${poi_tmp}" "${exceptsql}" "${tmp_sql}" "${add_poi}" $partitions "${insert_daily_sql}" "${insert_dinein_sql}" $isHbase "$filesnum" "$dropsql" "$dws_device_catering_dinein_di" "$dws_device_lbs_poi_10type_di" "$udf_tmp" "$dw_base_poi_l1_geohash"
