...
CHECK_DATA "hdfs://ShareSdkHadoop/user/hive/warehouse/dm_mobdi_master.db/dwd_wifilist_explore_sec_di/day=${insert_day}"
...
###目标表
device_location_daily_incr=dw_mobdi_md.device_location_daily_incr
...
insert overwrite table $device_location_daily_incr partition(day='$insert_day', source_table='wifi_scan_list')
...
from
(
  select trim(lower(nvl(muid, ''))) as muid
...
(
  ...
  and source_table='wifi_scan_list'
) t2 on t1.muid=t2.muid;
"
