...
insert overwrite table $dw_device_location_di partition (day='$day', source_table='auto_location_info')
select trim(lower(device)) device,
       duid,
...
(
  ...
  from
  (
    select device, duid, lat, lon, time, processtime,
