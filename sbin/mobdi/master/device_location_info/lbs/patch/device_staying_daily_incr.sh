...
#output
device_staying_daily_incr=dm_mobdi_tmp.device_staying_daily_incr
...
with ranked_device_time as (
  select
  device, duid, lat, lon, time, processtime, country, province, city, area, street, plat, network, type, data_source, orig_note1, orig_note2, accuracy,apppkg,ga_abnormal_flag
  from
  (
    select
    ...
    row_number() over(partition by device, lat, lon, time order by level) as rank,apppkg,ga_abnormal_flag
    from (
      select device, duid,
      ...
      case when type = 'gps' then 1
           ...
           when type = 'ip' then 5
           else 999 end as level,apppkg,ga_abnormal_flag
      from $device_location_daily_incr
...
insert overwrite table $device_staying_daily_incr partition (day='$insert_day')
select
...
processtime, country, province, city, area, street, plat, network, type, data_source, orig_note1, orig_note2, accuracy, apppkg,ga_abnormal_flag
