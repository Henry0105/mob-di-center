...
auto_location_db=$(echo $dwd_auto_location_info_sec_di|awk -F '.' '{print $1}')
auto_location_tb=$(echo $dwd_auto_location_info_sec_di|awk -F '.' '{print $2}')
...
CHECK_DATA "hdfs://ShareSdkHadoop/user/hive/warehouse/$auto_location_db.db/$auto_location_db/day=${day}"
CHECK_DATA "hdfs://ShareSdkHadoop/user/hive/warehouse/$auto_location_db.db/$auto_location_db/day=${plus_1day}"
CHECK_DATA "hdfs://ShareSdkHadoop/user/hive/warehouse/$auto_location_db.db/$auto_location_db/day=${plus_2day}"
# ##########################################
...
from (
    ...
    from (
        ...
        from (
            ...
            left join (
                select * from $dim_geohash6_china_area_mapping_par
                where version='1000'
            ) geohash6_mapping
            on (get_geohash(lat, lon, 6) = geohash6_mapping.geohash_6_code)  --通过GEOHASH6关联
        ) geo6
        left join (
            select * from $dim_geohash8_china_area_mapping_par
            where version='1000'
        ) geohash8_mapping
