...
: '步骤
1. 写入0324规则数据,新建临时表，包括本次所有0324推荐设备数据    dm_device_rec_for_0324_pre_table
2. 新建临时表，包括本次所有0324推荐设备数据+pre表中没有但在0324表次新分区中有，且未发回安装数据且不在黑名单的设备     dm_device_rec_for_0324_pre_second_table
3. 更新至0324规则结果表最新分区，增加增改、删、不变的状态     dm_device_rec_for_0324_table
'
...
#获取最新分区
full_par=`hive -e "show partitions dm_device_rec_for_0324;"  |sort -rn | awk -F "=" '{print $2}' | head -n 1`
...
--name "rta_device_rec_for_0324" \
--deploy-mode cluster \
--class com.youzu.mob.rta.rta_device_rec_for_0324 \
