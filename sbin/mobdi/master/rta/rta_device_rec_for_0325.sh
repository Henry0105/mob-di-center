...
: '步骤
1. 新建0325临时结果表，包括付费天数及距今付费间隔时间维度的设备     dm_device_rec_for_0325_pre_table
2. 更新至0325规则结果表最新分区，增加增改、删、不变的状态     dm_device_rec_for_0325_table
'
...
#获取最新分区
full_par=`hive -e "show partitions $dm_device_rec_for_0325;"  |sort -rn | awk -F "=" '{print $2}' | head -n 1`
...
--name "rta_device_rec_for_0325" \
--deploy-mode cluster \
--class com.youzu.mob.rta.rta_device_rec_for_0325 \
