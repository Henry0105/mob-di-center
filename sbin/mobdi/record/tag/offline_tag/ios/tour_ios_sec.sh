...
#                           示例：-f [a:a1,b:b1:,] 或者 --fields [a:a1,b:b1:,]    ,其中a,b为原标签在feature字段中的名称，并且b字段按照分隔符逗号（中英文）切分。a1，b1为a，b在feautre字段中的重命名名称
...
/usr/bin/spark2-submit  --executor-memory 15G   \
	...
	/home/dba/lib/offlineLabel-v0.1.0-jar-with-dependencies.jar  --partition ${day} --pk ifid --filenum 2000 --fields [level:level,cate1:cate1:,] \
	--lbstype 9 --windowtime ${timewindow} --sourcetable dm_mobdi_master.dwd_sdk_lbs_daily_poi_ios_sec_di --targetable rp_mobdi_app.timewindow_offline_profile_ios_v2_sec
...
insert overwrite table $timewindow_offline_profile_ios_v2_sec partition(flag=9,day=$day,timewindow=$timewindow)
select ifid,feature,cnt from $timewindow_offline_profile_ios_v2_sec where flag=9 and day=$day and timewindow=$timewindow
"