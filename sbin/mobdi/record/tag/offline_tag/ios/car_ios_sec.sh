...
# -f, --fields <value>     （必须参数） 需要计算的标签名称和重命名（仅允许添加一个在源表中不存在的字段total,用于计算device活跃天数）和分隔符(只支持单字符分隔符，在碰到中英文的逗号的任意一种，均按照中英文各切分一次)，示例：-f [a:a1,b:b1:,] 或者 --fields [a:a1,b:b1:,],其中a,b为原标签在feature字段中的名称，并且b字段按照分隔符逗号（中英文）切分
...
/usr/bin/spark2-submit  --executor-memory 15G   \
	...
	--sourcetable dm_mobdi_master.dwd_sdk_lbs_daily_poi_ios_sec_di \
	--targetable rp_mobdi_app.timewindow_offline_profile_ios_v2_sec
...
insert overwrite table $timewindow_offline_profile_ios_v2_sec partition(flag=3,day=$day,timewindow=$timewindow)
select ifid,feature,cnt from $timewindow_offline_profile_ios_v2_sec where flag=3 and day=$day and timewindow=$timewindow
"