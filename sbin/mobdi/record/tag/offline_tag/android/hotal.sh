...
/usr/bin/spark2-submit  --executor-memory 15G   \
	...
	--conf "spark.default.parallelism=16001" \
	...
	--lbstype 6 --windowtime ${timewindow} --pk device --sourcetable dm_mobdi_master.sdk_lbs_daily_poi --targetable rp_mobdi_app.timewindow_offline_profile_v2
...
insert overwrite table rp_mobdi_app.timewindow_offline_profile_v2 partition(flag=6,day=$day,timewindow=$timewindow)
select device,feature,cnt from rp_mobdi_app.timewindow_offline_profile_v2 where flag=6 and day=$day and timewindow=$timewindow
"
