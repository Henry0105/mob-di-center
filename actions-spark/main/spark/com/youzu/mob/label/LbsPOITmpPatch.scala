  ...
  def main(args: Array[String]) {
    ...
    if (count > 0) {
      if (isHbase) {
        ...
        val inputPoiSQL = "SELECT g7,lat,lon,name,type,type_id FROM dw_mobdi_md.dw_base_poi_l1_geohash"
      ...
      unionM =
        """
          ...
          |    lat,lon,type_1,type_2,type_3,type_4,type_5,type_6,type_7,type_8,type_9,type_10 from dw_mobdi_md.udf_tmp
          |  union all
          |select geohash_center5,geohash_center6,geohash_center7,geohash_center8,
...
//      hiveContext.sql(
//        s"""insert overwrite table dm_mobdi_master.device_catering_dinein_detail partition(day=${partition})
//           |select * from insert_dinein_repartition
//         """.stripMargin)
      hiveContext.sql(
        s"""insert overwrite table dw_mobdi_md.device_lbs_poi_daily_incr partition(day=${partition})
